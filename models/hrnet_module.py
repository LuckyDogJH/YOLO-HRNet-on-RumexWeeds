...
class HighResolutionNet(nn.Module):
    def __init__(self, input_channel, base_channel: int = 32, num_joints: int = 1):
        super().__init__()
        # Stem
        self.conv_init = nn.ModuleList([nn.Conv2d(input_channel[0], 256, kernel_size=1, stride=1, padding=0, bias=False),
                                       nn.Conv2d(input_channel[1], 256, kernel_size=1, stride=1, padding=0, bias=False),
                                       nn.Conv2d(input_channel[2], 256, kernel_size=1, stride=1, padding=0, bias=False)])
        ...
        self.transition2 = nn.ModuleList([
            self.Identity,  # None,  - Used in place of "None" because it is callable
            self.Identity,  # None,  - Used in place of "None" because it is callable
            nn.Sequential(
                nn.Sequential(
        ...
        for name, parameters in self.named_parameters():
            if 'weight' in name:
                if len(parameters.size()) < 2:
                    nn.init.kaiming_normal_(parameters.unsqueeze(0), mode='fan_in', nonlinearity='relu')
                else:
                    nn.init.kaiming_normal_(parameters, mode='fan_out', nonlinearity='relu')


    def _hrnet_forward_once(self, x, layer_ind):
        x = self.conv_init[layer_ind](x)
        x = self.bn1(x)
        x = self.relu(x)
